...
def f(d_ini, folder_out, path_data, path_obs, cv_i, **kwargs):
    ...
    class MySpotSetup(object):
        ...
        def parameters(self):
            ...
            if len(kwargs)!=0:
                ...
                if ("wang" in kwargs.values()) or ("triangular" in kwargs.values()):
                    ...
                    if kwargs["par_scenario"] == "wide":
                        par3_sample = parameter_dist(30, 45, cv_i, "MxDT") # Wide range scenario
                    elif kwargs["par_scenario"] == "narrow":
                        par3_sample = parameter_dist(32, 38, cv_i, "MxDT") # Narrow range scenario
                    else:
                        raise KeyError("Missing prior parameter range test settings!")
                    # Append the parameter values
                    ...
                    # Append the parameter values
                    param_list.append(par1_sample)
                    # par3: Tdmax, the maximum temperature beyond which the development rate is null.
                    if kwargs["par_scenario"] == "wide":
                        par3_sample = parameter_dist(30, 45, cv_i, "MxDT") # Wide range scenario
                    elif kwargs["par_scenario"] == "narrow":
                        par3_sample = parameter_dist(32, 38, cv_i, "MxDT") # Narrow range scenario
        ...
        def objectivefunction(self, simulation, evaluation, params=None):
            ...
            return like
    # Extract the supplied algorithm choice 
    if "algorithm_choice" not in kwargs.keys(): 
        raise KeyError("The algorithm to use is not specified as it can not be found on the key of input dictionary")
    else:
        algorithm_choice = kwargs["algorithm_choice"] # Obtain the str that represent the algorithm used during the parameter optimization
    # Set the log output file path
    log_out = os.path.join(folder_out, algorithm_choice + '_CV' + str(cv_i-1)+
                      '.log')  # str(d_ini[0]) + '_' + str(d_ini[len(d_ini) - 1])
    sys.stdout = open(log_out, 'w') # System wide print 
    my_spot_setup = MySpotSetup(spotpy.objectivefunctions.rmse)
    print('... before sampler...')
    # Set the main output file path name (no suffix), i.e. a database file that store all simulations and sampled parameters during optimization  
    db_out = os.path.join(folder_out, algorithm_choice + "_CV" + str(cv_i-1)) # str(d_ini[0]) + '_' + str(d_ini[len(d_ini) - 1]) 
    # Define the number of maximum repetitions
    rep = 50000 # This value can be changed
    # We start the sampler and set some optional algorithm specific settings
    # The SA, SCEUA and MLE are all non-Bayesian approaches
    if algorithm_choice == "sa":
        # Create the SA (Simulation Annealling) sampler 
        sampler = spotpy.algorithms.sa(my_spot_setup, dbname=db_out,
                                          dbformat='csv')  # Initialize the model with a setup file
        # Place where the SA optimization is being implemented
        sampler.sample(rep) # Default setting to run the model
    elif algorithm_choice == "sceua":
        # Create the SCEUA (shuffle complex evolution university Arizona) sampler 
        sampler = spotpy.algorithms.sceua(my_spot_setup, dbname=db_out,
                                          dbformat='csv') # Initialize the model with a setup file
        sampler.sample(rep, ngs=40) # Default setting to run the model except for the "ngs" parameter (increased from 20 to 40)
    elif algorithm_choice == "mle": 
        # Create the MLE (Maximum Likelihood Estimation) sampler 
        sampler = spotpy.algorithms.mle(my_spot_setup, dbname=db_out,
                                          dbformat='csv') # Initialize the model with a setup file
        sampler.sample(rep) # Default setting to run the model
    elif algorithm_choice == "fast": # This algorithm is not intended for parameter calibration/estimation but only for a sensitivity analysis
        # Obtain the number of parameters involved in the model
        if ("wang" in kwargs.values()) or ("triangular" in kwargs.values()):
            para_N = 4
        elif ("GDD_Richardson" in kwargs.values()) or ("sigmoid" in kwargs.values()):
            para_N = 3
        # Compute the required parameter iterations based on the following equation:
        # N=(1+4M**2(1+(k−2)d))k # By default, M = 4, d =2 , k=para_N
        N_iter = (1+4*4**2*(1+(para_N-2)*2)) * para_N
        # Create the FAST (Fourier Amplitude Sensitivity Test) sampler 
        sampler = spotpy.algorithms.fast(my_spot_setup, dbname=db_out,
                                          dbformat='csv') # Initialize the model with a setup file
        sampler.sample(N_iter) # Run the model
    # sys.stdout.close()
    ## Plot Section
    # Get all the simulation results of the sampler
    if algorithm_choice != "fast": # The way to load the simulation results for methods other than the FAST
        results = sampler.getdata() # Load the results 
        # Make the parameter interaction plot of your best 10% samples
        posterior=spotpy.analyser.get_posterior(results, percentage=10) # Extract the best 10% of parameter sampling results
        spotpy.analyser.plot_parameterInteraction(posterior,fig_name=folder_out + '/Parameter_interaction.png') # The parameter interaction plot of your best 10% samples
        # Make the parameter evolving plot during the optimization
        spotpy.analyser.plot_parametertrace_algorithms(results, [algorithm_choice], my_spot_setup, fig_name=folder_out + '/Parameter_convergence.png')
        # Make the Griewank function of parameter combination over a heat map. Sampled parameter combinations during optimizations are shown with the Griewank function (able to see if a optimum is found). 
        spotpy.analyser.plot_heatmap_griewank(results,[algorithm_choice], fig_name= folder_out + '/Parameter_Griewank.png')
        # Make the traced objective function plot
        spotpy.analyser.plot_objectivefunctiontraces(results,my_spot_setup.evaluation(),[algorithm_choice], fig_name=folder_out + '/ObjectiveFunction_trace.png')
    else:
        results = spotpy.analyser.load_csv_results(db_out) # An unique way to load simulation results for FAST
        spotpy.analyser.plot_fast_sensitivity(results, number_of_sensitiv_pars=para_N, fig_name= folder_out + '/Sensitivity_Index.png') 
